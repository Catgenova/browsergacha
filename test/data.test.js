...
test('no two abilities are mechanically identical', () => {
  ...
  const named = new Set(['coral', 'emily', 'toll', 'echo', 'javarious',
    ...
    'lucian', 'franz', 'carl', 'esmerelda', 'slick', 'samuels', 'lin', 'koe']);
  const seen = new Map();
  for (const h of heroes) for (const a of h.abilities) {
...
test('human sects hold real humans, once each, with their numbers', () => {
  const expected = {
    cryst: { number: 1, members: ['polarus', 'echo', 'florence', 'andrew', 'ari', 'cain', 'bit', 'tanner'] },
    ...
    firetroupe: { number: 5, members: ['lucian', 'franz', 'carl', 'esmerelda', 'slick', 'samuels', 'lin', 'koe'] },
  };
