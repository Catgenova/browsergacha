...
const Blessing = (() => {
  const KINDS = {
    ...
    godtouched: {
      id: 'godtouched', name: 'Godtouched', icon: '✦',
  ...
  // The roster-card mark: the kind's glyph in its own colour.
  function iconHtml(kind) {
    const b = KINDS[kind];
    if (!b) return '';
    return `<span class="card-blessing blessing-${b.id}"` +
      ` title="${b.name} — ${b.blurb}">${b.icon}</span>`;
  }

  return { KINDS, roll, of, statMult, iconHtml };
})();
